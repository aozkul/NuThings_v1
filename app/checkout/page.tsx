  ...
  if (!hasItems && done) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        {done?.order_id &&
          <p className="mt-2 text-sm text-neutral-500">Sipariş No: <span className="font-mono">{done.order_id}</span>
          </p>}
      </div>
    );
  }
          ...
          <section className="lg:col-span-3 rounded-lg border p-4 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input className="w-full border rounded-md h-10 px-3" value={form.firstName}
                       onChange={e => onChange("firstName", e.target.value)}/>
              </div>
              <div>
                <label className="block text-xs mb-1">{tt('last_name')}</label>
                <input className="w-full border rounded-md h-10 px-3" value={form.lastName}
                       onChange={e => onChange("lastName", e.target.value)}/>
              </div>
              <div className="sm:col-span-2">
                <label className="block text-xs mb-1">{tt('email')}</label>
                <input type="email" className="w-full border rounded-md h-10 px-3" value={form.email}
                       onChange={e => onChange("email", e.target.value)}/>
              </div>
              <div className="sm:col-span-2">
                <label className="block text-xs mb-1">{tt('address')}</label>
                <input className="w-full border rounded-md h-10 px-3" value={form.address}
                       onChange={e => onChange("address", e.target.value)}/>
              </div>
              <div>
                <label className="block text-xs mb-1">{tt('city')}</label>
                <input className="w-full border rounded-md h-10 px-3" value={form.city}
                       onChange={e => onChange("city", e.target.value)}/>
              </div>
              <div>
                <label className="block text-xs mb-1">{tt('zip')}</label>
                <input className="w-full border rounded-md h-10 px-3" value={form.zip}
                       onChange={e => onChange("zip", e.target.value)}/>
              </div>
              <div>
                <label className="block text-xs mb-1">{tt('country')}</label>
                <select className="w-full border rounded-md h-10 px-3" value={form.country}
              ...
              <label className="flex items-center gap-2 sm:col-span-2 mt-2">
                ...
                <span className="text-sm">Mesafeli satış sözleşmesini ve <a className="underline" href="/agb"
                                                                            target="_blank">şartları</a> kabul ediyorum.</span>
              </label>
            </div>
          ...
          <aside className="lg:col-span-2 space-y-4">
                ...
                {items.map((it: any) => (
                  <li key={it.id} className="py-3 flex items-center gap-3">
                    {it.image ? (
                      ...
                      <div
                        className="h-14 w-14 rounded-md bg-neutral-100 grid place-items-center text-xs ring-1 ring-neutral-200">{tt('no_image')}</div>
                    )}
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 inline-flex items-center gap-1.5">
                        ...
                        <input className="w-10 text-center border rounded-md h-6" value={it.quantity}
                               onChange={(e) => updateQty(it.id, Math.max(1, parseInt(e.target.value || '1', 10) || 1))}/>
                        <button className="h-6 w-6 grid place-items-center rounded-md border"
                                onClick={() => updateQty(it.id, (it.quantity || 1) + 1)}>+
                        </button>
                        <button className="ml-2 text-xs text-red-600 underline"
                                onClick={() => remove(it.id)}>{tt('remove')}</button>
                      </div>
                    </div>
            ...
            <section className="rounded-lg border p-4">
              ...
              <div className="w-full max-w-sm">
                {valid ? (
                  <CheckoutPayPal amount={totalStr} referenceId={`live-${form.email || 'guest'}-${Date.now()}`}
                                  onApprove={handleApprove}/>
                ) : (
                  <div
                    className="w-full h-[45px] grid place-items-center border rounded-md text-sm text-neutral-400 bg-neutral-50">
                    {tt('fill_form_first')}
                  </div>
                )}
              </div>
